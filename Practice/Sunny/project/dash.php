...
<script> 
  $(function () //It ensures that the code inside the function is executed once the DOM is fully loaded.
   {
...
echo '<span class="pull-right top title1" >
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;Hello,</span> <a href="#" class="log log1">'.$name.'</a>&nbsp;|&nbsp;
        <a href="logout.php?q=account.php" class="log"><span class="glyphicon glyphicon-log-out" aria-hidden="true"></span>
...
<?php if(@$_GET['q']==0) { // means i'm on admin home page

$result = mysqli_query($con,"SELECT * FROM quiz where adminEmail='$email' ORDER BY date DESC") or die('Error'); // getting all exams
...
$c=1;
while($row = mysqli_fetch_array($result)) { // traversing each row of result
  // Inside the loop, you can access individual columns in the current row using array notation, such as $row['column_name'].
	$title = $row['title'];
	$total = $row['total'];
	$sahi = $row['sahi'];
  $time = $row['time'];
	...
	echo '<tr>
          ...
          <td>'.$time.'&nbsp;min</td>
	        <td>
            <b>
              <a href="update.php?q=rmquiz&eid='.$eid.'" class="pull-right btn btn-danger" style="margin:0px; border-radius:0%;">
